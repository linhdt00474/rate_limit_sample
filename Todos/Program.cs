...
//rateLimiterOptions.AddConcurrencyLimiter("current", options =>
//{
...
//    options.QueueLimit = 3;// maximum number of request can be in queue at a time
//});

//Handle the connection based on token bucket algorithm
...
//rateLimiterOptions.AddTokenBucketLimiter("current", options =>
//{
//    options.TokenLimit = 5; //maximum number of tokens that can be in token bucket
//    options.QueueProcessingOrder = QueueProcessingOrder.NewestFirst; // order for queue processing
//    options.QueueLimit = 1; //maximum number of unprocessed tokens waiting in queue
//    options.ReplenishmentPeriod = TimeSpan.FromSeconds(5); //specifies how often tokens can be replenished. Replenishing is triggered either by set autoReplenishment is true, or by calling TryReplenish()
//    options.TokensPerPeriod = 1; // number of tokens can be added to the token bucket in replenishing process, token count will not exceed tokenLimit
...
rateLimiterOptions.AddFixedWindowLimiter("current", options =>
{
...
//rateLimiterOptions.AddSlidingWindowLimiter("current", options =>
//{
